...
- **Live metrics** — per-node CPU/memory badges stream onto the canvas over a WebSocket while the run is live, plus per-DB write counts and a Metrics drawer table. Toggle **⚡ Load source** on any service/LB node in the Inspector to give it a rate (a ⚡N/s chip appears on the node); set a global duration and press **Run load** to fire k6 at every marked source at once — the Metrics drawer shows per-target achieved vs target RPS and dropped iterations, with saturated targets highlighted. Set a source's **body size (KB)** (1–1024) to have k6 post a fat payload, and a service's **payload sensitivity (ms/KB)** to add proportional latency per received kilobyte — together these let a large body saturate a service. The sweet spot for visible effect is **16–128 KB**; at ~1 MB transport overhead dominates and the latency signal becomes noisy.
